...
function RautoMap() {
    ...
    if (!game.global.preMapsActive && game.global.mapsActive) {
        ...
        if (selectedMap == game.global.currentMapId && (!getCurrentMapObject().noRecycle && (doDefaultMapBonus || vanillaMapatZone || RdoMaxMapBonus || RshouldFarm || RneedPrestige))) {
            ...
            if (!RshouldDoMaps && (game.global.mapGridArray[game.global.mapGridArray.length - 1].special == targetPrestige && game.mapUnlocks[targetPrestige].last >= (game.global.world + extraMapLevels - 9))) {
                repeatClicked();
            }
